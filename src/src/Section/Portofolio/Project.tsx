    ...
    <div className="p-10 lg:pl-30 lg:pr-30">
      ...
      <div className="flex justify-center mb-6 ">
        <button
          ...
          className={`px-4 py-2 text-lg font-semibold rounded-md mx-2 transition-all ${
            activeTab === "project" ? "bg-custom-brown text-white" : "bg-zinc-800 text-gray-300"
          }`}
        >
          Project
        </button>
        <button
          ...
          className={`px-4 py-2 text-lg font-semibold rounded-md mx-2 transition-all ${
            activeTab === "sertifikasi" ? "bg-custom-brown text-white" : "bg-zinc-800 text-gray-300"
          }`}
        >
          Sertifikasi
        </button>
              ...
              <div className="absolute right-0 mt-2 w-40 bg-zinc-900 border border-zinc-700 rounded-lg shadow-lg overflow-hidden">
                {projectTypes.map((type) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2 capitalize hover:bg-zinc-800 ${
                      selectedType === type ? "border-b-2 border-custom-brown text-white" : "text-slate-300"
                    }`}
                  >
                    {type}
                  </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredItems.length > 0 ? (
          filteredItems.map((item) => (
            <div key={item.id} className="p-4 bg-zinc-950 rounded-lg shadow-md transition-all duration-300 hover:shadow-lg">
              <div className="text-center">
                <img src={item.image} alt={item.title} className="w-full h-100% object-cover rounded-md mb-2" />
                <span
          ...
          <div className="text-slate-300 text-center py-12 col-span-full">Tidak ada data yang ditemukan</div>
        )}
      </div>
    </div>
  );
